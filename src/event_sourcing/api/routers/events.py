...
from ...application.commands.salesforce import AsyncProcessSalesforceEventCommand
from ...application.commands.handlers.async_process_salesforce_event import AsyncProcessSalesforceEventCommandHandler
...
async def process_salesforce_event(
    ...
    handler: AsyncProcessSalesforceEventCommandHandler = Depends(DependencyService.get_async_process_salesforce_event_command_handler)
) -> Dict[str, Any]:
    """
    ...
    logger.info(f"Successfully triggered async processing for Salesforce event: {command.command_id}")
