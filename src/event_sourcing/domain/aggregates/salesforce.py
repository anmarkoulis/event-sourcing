...
class SalesforceAggregate(BaseAggregate):
    ...
    def validate_event_ordering(self, event: DomainEvent) -> bool:
        ...
        if event.event_type == "Created":
            ...
            if self.version > 0:
                logger.warning(f"Creation event received for existing {self.entity_name} {self.aggregate_id}")
                return False
            return True
        else:
            # Non-creation events require existing entity
            if self.version == 0:
                logger.warning(f"Non-creation event received for non-existent {self.entity_name} {self.aggregate_id}")
                return False
            return True
    ...
    def handle_backfill_scenario(self, event: DomainEvent) -> None:
        ...
        if not self.validate_event_ordering(event):
            logger.info(f"Triggering backfill for {self.entity_name} {self.aggregate_id}")
            # This will be handled by the command handler
            # We just log the scenario here
            logger.info(f"Backfill needed for {self.entity_name} {self.aggregate_id}")
