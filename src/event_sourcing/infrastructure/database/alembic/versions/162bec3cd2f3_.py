...
Revision ID: 162bec3cd2f3
Revises:
Create Date: 2025-08-04 05:52:19.991418
...
revision = "162bec3cd2f3"  # pragma: allowlist secret
down_revision = None
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_user_event_stream_revision"),
        "user_event_stream",
        ["revision"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_user_event_stream_revision"), table_name="user_event_stream"
    )
    op.drop_index(
