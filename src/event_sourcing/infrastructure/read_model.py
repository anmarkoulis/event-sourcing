...
from ..dto.client import ClientDTO
from ..application.queries.base import SearchClientsQuery
from .database.session import DatabaseManager, AsyncDBContextManager
from .database.models.client import Client as ClientModel
...
class PostgreSQLReadModel(ReadModel):
    ...
    async def save_client(self, client_data: Dict[str, Any]) -> None:
        ...
        async with AsyncDBContextManager(self.database_manager) as session:
            ...
            existing_client = await session.execute(
                select(ClientModel).where(ClientModel.aggregate_id == aggregate_id)
            )
    ...
    async def search_clients(self, query: SearchClientsQuery) -> List[ClientDTO]:
        ...
        base_query = base_query.order_by(ClientModel.created_at.desc()).offset(offset).limit(query.page_size)
    ...
    async def get_client(self, client_id: str) -> Optional[ClientDTO]:
        ...
        query = select(ClientModel).where(ClientModel.aggregate_id == client_id)
    ...
    async def get_clients_by_status(self, status: str, limit: int = 100) -> List[ClientDTO]:
        """Get clients by status"""
        logger.info(f"Getting {limit} clients with status {status}")
        
        query = select(ClientModel).where(
            ClientModel.status == status
        ).order_by(ClientModel.created_at.desc()).limit(limit)
    ...
    async def get_clients_by_parent(self, parent_id: str, limit: int = 100) -> List[ClientDTO]:
        """Get clients by parent ID"""
        logger.info(f"Getting {limit} clients with parent {parent_id}")
        
        query = select(ClientModel).where(
            ClientModel.parent_id == parent_id
        ).order_by(ClientModel.created_at.desc()).limit(limit)
