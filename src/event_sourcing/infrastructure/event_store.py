...
from .database.models.event import Event as EventModel
...
class PostgreSQLEventStore(EventStore):
    ...
    async def get_events(self, aggregate_id: str, aggregate_type: str, 
        ...
        async with AsyncDBContextManager(self.database_manager) as session:
            ...
            logger.info(f"Retrieved {len(domain_events)} events for {aggregate_type} {aggregate_id}")
    ...
    async def get_events_by_type(self, event_type: str, limit: int = 100) -> List[DomainEvent]:
        ...
        ).order_by(EventModel.timestamp.desc()).limit(limit)
    ...
    async def get_events_by_source(self, source: str, limit: int = 100) -> List[DomainEvent]:
