...
from ..commands.aggregate import ReconstructAggregateCommand, AsyncUpdateReadModelCommand, AsyncPublishSnapshotCommand
from ..commands.handlers.reconstruct_aggregate import ReconstructAggregateCommandHandler
from ..commands.handlers.async_update_read_model import AsyncUpdateReadModelCommandHandler
from ..commands.handlers.async_publish_snapshot import AsyncPublishSnapshotCommandHandler
...
async def reconstruct_aggregate_async(
    ...
    logger.info(f"Successfully reconstructed aggregate asynchronously: {command_id}")
    return snapshot


async def trigger_next_steps(aggregate_id: str, aggregate_type: str, snapshot: Dict[str, Any]) -> None:
...
def reconstruct_aggregate_task(
    ...
    logger.info(f"Starting Celery task for reconstruct aggregate command: {command_id}")
    
    # Convert async function to sync for Celery
    reconstruct_aggregate_async_sync = async_to_sync(reconstruct_aggregate_async)
    ...
    logger.info(f"Completed Celery task for reconstruct aggregate command: {command_id}")
    return snapshot 