...
from ..aggregate import UpdateReadModelCommand
...
class UpdateReadModelCommandHandler:
    ...
    async def handle(self, command: UpdateReadModelCommand) -> None:
        ...
        logger.info(f"Updating read model for: {aggregate_type} {aggregate_id}")
        ...
        if aggregate_type == "client":
            ...
            logger.warning(f"Unknown aggregate type for read model: {aggregate_type}")
            return
        
        logger.info(f"Successfully updated read model for: {aggregate_type} {aggregate_id}") 