...
from ..salesforce import AsyncProcessSalesforceEventCommand
from event_sourcing.application.tasks.process_salesforce_event import process_salesforce_event_task
...
class AsyncProcessSalesforceEventCommandHandler:
    ...
    async def handle(self, command: AsyncProcessSalesforceEventCommand) -> None:
        """Handle the async command by triggering a Celery task"""
        logger.info(f"Triggering Celery task for command: {command.command_id}")
